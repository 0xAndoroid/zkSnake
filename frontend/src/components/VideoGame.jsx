...
const VideoGame = ({ snakeArr = [] }) => {
  ...
  const isSnakeSquare = (x, y) => {
    return snakeArr.some(square => square[0] === x && square[1] === y);
  };
  ...
  const grid = Array.from({ length: 10 }, (_, y) => (
    <div key={y} className="flex">
      {Array.from({ length: 10 }, (_, x) => (
        <div
          key={`${x},${y}`}
          className={`w-14 h-14 inline-flex justify-center items-center ${isSnakeSquare(x, y) ? 'bg-green-500' : 'bg-transparent'} border border-gray-300`}
        ></div>
      ))}
    </div>
  ));
